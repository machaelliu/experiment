...
void co_log_info(string content) {
  content += "\n";
  int fd = open("./info.log", O_WRONLY|O_CREAT|O_APPEND); 
  if(fd == -1) {
  ...
  if (write(fd, content.c_str(), len) != len) {
...
void co_yield_env( stCoRoutineEnv_t *env )
{
  ...
  //fprintf(stderr, "tag14 222\n");
  //fflush(stderr);
  //co_log_info("tag16");
	co_swap( curr, last);
  //co_log_info("tag17");
  //fprintf(stderr, "tag15 222\n");
}
...
int co_poll_inner( stCoEpoll_t *ctx,struct pollfd fds[], nfds_t nfds, int timeout, poll_pfn_t pollfunc)
{
  ...
  //fprintf(stderr, "tag10 before co_yield_env\n");
	co_yield_env( co_get_curr_thread_env() );
  //fprintf(stderr, "tag11 after co_yield_env\n");
